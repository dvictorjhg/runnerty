...
class Plan {
  ...
  async createChainSerie(inputIterable, chain, process, custom_values_overwrite) {
    const inputIterableLength = inputIterable.length;
    for (let i = 0; i < inputIterableLength; i++) {
  ...
  async execSerie(chains, inputIterable, chain) {
    ...
    const iterations = inputIterable.length;
  ...
  async scheduleChainIterableParallel(inputIterable, chain, process, custom_values_overwrite) {
    try {
      ...
      const iterations = process.childs_chains.length;
      let chainsToExecLength = process.childs_chains.length;
      while (chainsToExecLength--) {
        const options = {
          iterations,
          iteration: iterations - chainsToExecLength,
      ...
      Promise.all(process.childs_chains).then(chains => {
        for (const chain of chains) {
          chain.killRunnertyOnForcedInitChainsStop();
        }
      });
    } catch (err) {
      throw err;
