...
class Process {
    ...
    if (this.hasOwnProperty('notifications') && this.notifications !== undefined) {
      if (this.notifications.hasOwnProperty(event)) {
        ...
        this.notifications[event].map(_notification => {
          notificationsPromises.push(_notification.notificate(event, this.values()));
        });
  ...
  async end(notificate, writeOutput) {
    ...
    if (writeOutput) {
      await this.write_output({ context: 'end' });
    }
    await this.setOutputShare();
    this.historicize('end');
  }
  ...
  async startChildChainsDependients() {
    ...
    if (this.output_share) {
      const options = {
        ...
        source: 'process.startChildChainsDependients.this.output_share'
      };
  ...
  async setOutputShare() {
    if (this.hasOwnProperty('output_share') && this.output_share) {
      const options = {
        ignoreGlobalValues: false,
        source: 'process.setOutputShare.this.output_share'
      };

      console.log('process.setOutputShare.this.output_share', {
        output_share: this.output_share,
        values: this.values()
      });
      ...
      this.execute_output_share.forEach(valOS => {
        ...
        console.log('process.setOutputShare.this.output_share', _valOS);
  ...
  async error(notificate, writeOutput, willRetry) {
    ...
    if (writeOutput) {
      this.write_output({ context: 'error' });
    }

    await this.setOutputShare();

    this.historicize('error');
  }
  ...
  async generateOutput(output, context) {
    try {
      if (output) {
        const _output = await recursiveObjectInterpreter(output, this.values(), {
          source: 'process.generateOutput._output',
          context
        });
  ...
  async write_output({ context } = {}) {
    try {
      if (this.output instanceof Array) {
        let outputCountItems = this.output.length;
        const promisesGO = [];
        while (outputCountItems--) {
          promisesGO.push(this.generateOutput(this.output[outputCountItems], context));
        }
        await Promise.all(promisesGO);
      } else {
        await this.generateOutput(this.output, context);
      }
    } catch (err) {
