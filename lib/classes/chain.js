...
class Chain {
  ...
  async init() {
    try {
      ...
      this.custom_values_resolved = await recursiveObjectInterpreter(this.custom_values, this.values(), {
        source: 'chain.init.this.custom_values_resolved'
      });
      this.execId = await recursiveObjectInterpreter(this.execId, this.values(), {
        source: 'chain.init.this.execId'
      });
  ...
  values() {
    const chain_values = {
      ...
      CHAIN_RETRIES_COUNT: this.retries_count,
      CHAIN_RETRIES: this.retries,
      CHAIN_ITERATIONS: this.iterations,
      CHAIN_ITERATION: this.iteration,
      ...
      if (this.hasOwnProperty('notifications') && this.notifications !== undefined) {
        if (this.notifications.hasOwnProperty(event)) {
          ...
          while (notificationsLength--) {
            promNotif.push(this.notifications[event][notificationsLength].notificate(event, this.values()));
          }
  ...
  async historicize(event) {
    ...
    if (event === 'start' && this?.meta?.extra_id) {
      values.extraId = await recursiveObjectInterpreter(this?.meta?.extra_id, this.values(), {
        source: 'chain.historicize.values.extraId'
      });
    }
  ...
  async error() {
    if (!this.isErrored()) {
      ...
      if (this.isCompletelyFinished()) this.killRunnertyOnForcedInitChainsStop();
      this.retrying = false;
    }
  }
  ...
  async end() {
    if (!this.isEnded()) {
      this.ended_at = new Date().toISOString();
      this.status = 'end';
      this.causedByAnProcessError = this.causedByAnProcessError || false;
      const duration = chronometer(this.hr_started_time);
      this.duration_seconds = duration[0];
      this.duration_humanized = duration[1];
  ...
  async start(options) {
    if (options.inputIteration) {
      ...
      this.execute_input = await recursiveObjectInterpreter(this.input[0], this.values(), {
        source: 'chain.start.this.execute_input'
      });
    }

    // Refresh custom_values_resolved:
    this.custom_values_resolved = await recursiveObjectInterpreter(this.custom_values, this.values(), {
      source: 'chain.start.this.custom_values_resolved'
    });
  ...
  /**
   * Determines if the chain has completely finished its execution.
   *
   * Checks three conditions:
   * 1. If the chain has not ended and is not in error state
   * 2. If the chain is in retry mode and has not exhausted all retry attempts
   * 3. If the chain is iterable (parallel or serie) and has not completed all iterations
   *
   * @returns {boolean} Returns true if the chain has completely finished, false otherwise
   */
  isCompletelyFinished() {
    if (!this.isEnded() && !this.isErrored()) {
      return false;
    }

    if (this.retrying && this.retries_count && this.retries_count < this.retries) {
      return false;
    }

    if (
      this.isIterable() &&
      (this.iterable === 'parallel' || (this.iterable === 'serie' && this.iteration < this.iterations))
    ) {
      return false;
    }
  ...
  /**
   * Handles the termination of Runnerty when all forced initialization chains have stopped.
   * This method checks if the current chain is part of the forced initialization chains,
   * removes it from the tracking list, and if all forced chains are complete,
   * terminates the process.
   *
   * @returns {void}
   */
  killRunnertyOnForcedInitChainsStop() {
    if (!runtime.forcedInitChainsIds?.length || !runtime.endOnforcedInitChainsIds) {
      return;
    }

    const chainIndex = runtime.forcedInitChainsIds.indexOf(this.id);
    if (chainIndex === -1) {
      return;
    }

    runtime.forcedInitChainsIds.splice(chainIndex, 1);

    // Exit process if all forced chains are done
