...
const chainSchema = require('/Users/codertydev/workspace/runnerty/schemas/schemas/4.0/chain.json');
const processSchema = require('/Users/codertydev/workspace/runnerty/schemas/schemas/4.0/process.json');
const configSchema = require('/Users/codertydev/workspace/runnerty/schemas/schemas/4.0/config.json');
...
module.exports.loadGeneralConfig = async function loadGeneralConfig(configFilePath) {
  try {
    ...
    const confRep = await recursiveObjectInterpreter(configLoad.config, undefined, {
      source: 'utils.loadGeneralConfig.confRep'
    });
...
module.exports.loadRemoteGeneralConfig = async function loadRemoteGeneralConfig(url, username, password) {
  ...
  try {
    ...
    validateConfig(configLoad);
    const repConfig = await recursiveObjectInterpreter(configLoad.config, undefined, {
      source: 'utils.loadRemoteGeneralConfig.repConfig'
    });
...
async function loadRemoteFile(url, username, password) {
  const options = {
    url: url,
    headers: { 'User-Agent': 'runnerty', Accept: 'application/json', 'Content-Type': 'application/json' },
    method: 'GET'
  };
...
async function recursiveObjectInterpreter(inputObject, objParams = {}, options = {}) {
  if (runtime.config?.interpreter_max_size) {
    options.maxSize = options.maxSize ?? runtime.config.interpreter_max_size;
  }
  console.log('recursiveObjectInterpreter', inputObject, objParams, options);
  return await interpreter.interpret(inputObject, objParams, options, runtime.config?.global_values);
}
...
async function loadExecutors(executorsPath, executors) {
  try {
    ...
    if (executorsLength > 0) {
      ...
      configSchema.properties.executors.items = items;
...
async function loadNotifiers(notifiersPath, notifiers) {
  try {
    ...
    if (notifiersLength !== 0) {
      ...
      configSchema.properties.notifiers.items = items;
...
async function loadTriggers(triggersPath, triggers) {
  try {
    ...
    if (triggersLength > 0) {
      ...
      configSchema.properties.triggers.items = items;
...
function setUpServer(server) {
  function errorHandler(err, req, res, next) {
    logger.error('error server:', { error: err });
    res.status(500);
    next();
  }
...
function consoleAjvErrors(schema, data, errors) {
  const output = betterAjvErrors(schema, data, errors, { format: 'cli', indent: 2 });
