...
async function _match(key, value, values) {
  let res = false;
  key = await recursiveObjectInterpreter(key, values, { source: 'depends-process._match.key' });
  value = await recursiveObjectInterpreter(value, values, { source: 'depends-process._match.value' });
  ...
  if (objToCheck instanceof Object) {
    ...
    if (keyVal instanceof Object) {
      ...
      key = await recursiveObjectInterpreter(key, values, {
        source: 'depends-process._checkCondition.key(keyVal instanceof Object)'
      });
      value = await recursiveObjectInterpreter(value, values, {
        source: 'depends-process._checkCondition.value(keyVal instanceof Object)'
      });
      ...
      key = await recursiveObjectInterpreter(key, values, { source: 'depends-process._checkCondition.key(else)' });
      keyVal = await recursiveObjectInterpreter(keyVal, values, {
        source: 'depends-process._checkCondition.keyVal(else)'
      });
      // String
